...
def ordenar_lis_str_inv(lista):
	new_orden = list(set(ordenar_lis_num([len(i)for i in lista])))[::-1]
...
def units_value(var_str):
	...
	ord_values_more_str = ordenar_lis_str_inv([i for i in values_more_str])
	for i,var_tab in enumerate(ord_values_more_str):
		len_var_tab = len(var_tab)
		len_var_str = len(var_str)
		if len_var_tab<=len(var_str):
			# print(var_tab,var_str[:len_var_tab])
			# print(var_tab,var_str[len_var_str-len_var_tab:],'\n')
...
def dependent_variable(line):
	if (('=' in line) == False) or ('%' == line[0]):
		return '',False
	else:
		if '=' in line:
			line = line.split('%')[0]
...
def frases(i,unit, var_str, palabra, frase, sig=4):
	if  unit == 'pu':
...
def matlab_grader_all(texto, variables_help = '', lista_variables = '', lista_variables_no_deseadas = '', q_random = 100, palabra = 'Question', frase = 'Find the value of the variable', sig = 4):
	...
	if '=' in texto:
		...
		for pos,txt in enumerate(text_line):
			line = txt
			var, value = dependent_variable(line)
			if value:
		...
		for i,var in enumerate(variables_random):
			...
			unit, var_str = units_value(var)
			fq,fm,f_sol = frases(i = i,unit=unit, var_str=var_str, palabra = palabra, frase =frase)
			new_text = fm.split('\n')
			# print(var,text_line[pos_f])
